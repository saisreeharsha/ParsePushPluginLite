var serviceName = 'ParsePushPluginLite';
      ...
      if(ParsePushPluginLite.DEBUG){
		...
		if(pn !== null){
			if(pushAction === 'OPEN' || pn.OPEN){
				...
				//
            ParsePushPluginLite.softTrigger(ParsePushPluginLite._openEvent, pn);
			} else{
				//
				...
				//
				var base = ParsePushPluginLite._receiveEvent;
				var customEventKey = ParsePushPluginLite._customEventKey;

				ParsePushPluginLite.softTrigger(base, pn);
				if(customEventKey && pn[customEventKey]){
					ParsePushPluginLite.softTrigger(base + ':' + pn[customEventKey], pn);
				}
			}
		}
   };
...
//
// give ParsePushPluginLite event handling capability so we can use it to trigger
...
var EventMixin = {
   _coldStartDelayMs: 1000,
    ...
    trigger: function(events) {
      if(this.DEBUG){
         console.log("enter ParsePushPluginLite.trigger: " + events);
      }
      ...
      if(this.DEBUG){
         console.log("exit ParsePushPluginLite.trigger: " + events);
      }
      return this;
   },
...
module.exports = poorManExtend(ParsePushPluginLite, EventMixin);
